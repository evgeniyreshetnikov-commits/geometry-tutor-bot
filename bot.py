...
def get_user_stats(context: ContextTypes.DEFAULT_TYPE) -> dict:
    return context.user_data.setdefault("stats", default_stats())


def add_stars(context: ContextTypes.DEFAULT_TYPE, amount: int) -> dict:
...
def grant_badges(context: ContextTypes.DEFAULT_TYPE) -> list[str]:
    stats = get_user_stats(context)
...
def maybe_badge_text(context: ContextTypes.DEFAULT_TYPE) -> str:
    new_badges = grant_badges(context)
    if not new_badges:
        return ""
...
async def progress_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(format_progress_text(get_user_stats(context)), parse_mode="HTML")
    ...
    if mode == "daily":
        ...
        if lowered == normalize_text(task["answer"]):
            ...
            if stats.get("last_daily_date") != today:
                ...
                stats["streak"] += 1
                add_stars(context, 2)
            await update.message.reply_text(f"{random_praise()}\n\nЗадание дня выполнено! +2 ⭐{maybe_badge_text(context)}", reply_markup=MAIN_REPLY_MENU)
        else:
            stats["streak"] = 0
